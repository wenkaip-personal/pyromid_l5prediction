...
class CVAE(pl.LightningModule):
    ...
    ''' A conditional variational autoencoder with x-FC->z-FC->x_hat
        IN: 
        OUT:
    ...
    def _init_encoder(self, data_dim, cond_dim, hidden_layer_dim_list, latent_dim):
        layer_dim_list = [data_dim+cond_dim] \
    ...
    def _init_decoder(self, data_dim, cond_dim, hidden_layer_dim_list, latent_dim):
        layer_dim_list = [latent_dim+cond_dim] \
                       + hidden_layer_dim_list \
                       + [data_dim]
    ...
    def _step(self, batch):
        ...
        x_cat_c = torch.cat([x,c],dim=1)

        z_mu = self.encoder_mu(x_cat_c) 
        z_log_var = self.encoder_sigma(x_cat_c)
        ...
        loss, recon_loss, kl_div, pxz = self.loss(x, c, P, Q) #TODO: refactor loss
...
if __name__ == '__main__':
    ...
    args = CVAE.args_strings_to_lists(args)
    ...
    conditional_dim = np.prod(img_shape)
    data_dim = conditional_dim 

    # Initialize Model
    model = CVAE(data_dim, conditional_dim, **vars(args))
